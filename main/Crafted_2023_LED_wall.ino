...
uint8_t max_score = 255; // amount of points you need to score to win
...
const unsigned long screensaver_rate = 100;
...
void setup() {
  ...
  pinMode(MODE_PIN, INPUT_PULLUP); // to swith between modes
  ...
  switch (mode) {
    case 0:    // your hand is close to the sensor
        ...
        if (reset_screensaver) {
          matrix.setPassThruColor();
          matrix.fillScreen(0);
          screensaver_x = random(1, (pixel_x - 2));
          screensaver_y = random(1, (pixel_y - 2));
